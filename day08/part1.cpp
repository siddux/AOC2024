...
auto manhattan_distance(Coordinate a, Coordinate b) {
        ...
        for (size_t j = i + 1; j < locations.size(); ++j) {
            ...
            for (const auto& [x, y] : std::views::cartesian_product(
                ...
                std::views::iota(std::size_t(0), cols))) {
                    ...
                    int dist_to_A = manhattan_distance(P, A);
                    int dist_to_B = manhattan_distance(P, B);

                    // Check the distance ratio
                    if (dist_to_A == 2 * dist_to_B || dist_to_B == 2 * dist_to_A) {
                        result.insert(P);
                    }
            }
        }
